...
def pixel_maker_x1(pixel1, pixel2, pixel3, pixel4):
    try:
        #Increase the speed by getting rid of ints
        #But fix pixel blender
        a = pixel1
        c = pixel2
        g = pixel3
        i = pixel4
        ...
        e = pixel_blender([pixel1, pixel2, pixel3, pixel4])
...
def mapperx():
    try:
        ...
        if w % 2 == 0:
            ...
            w = (w - 1)
        print('Mapper:', 'H:', h, 'W:', w)
        map = {}
        block_counter = 0
        for by in range(h - 1):  # 0, 1, 2
            for bx in range(w - 1):  # 0, 1, 2
                block_counter += 1
                block_name = f'block_{block_counter}'
                block_coords = []
                for dy in range(2):
...
def mapper_to_array_single():
    try:
        ...
        print('Mapper To Array Issues:', e)
def upscale_python(input_path, output_path, n_size):
    img = Image.open(input_path)
    w, h = img.size
    pixels = img.load()
    new_img = Image.new(img.mode, (w * n_size, h * n_size))
        ...
        for x in range(w):
            ...
            for dy in range(n_size):
                for dx in range(n_size):
                    new_pixels[x * n_size + dx, y * n_size + dy] = color
    new_img.save(output_path)
def upscale_numpy(input_path, output_path, n_size):
    arr = np.array(Image.open(input_path))
    Image.fromarray(np.repeat(np.repeat(arr, n_size, axis=0), n_size, axis=1)).save(output_path)
    ...
    try:
        ...
        if len(pics) > 20:
            ...
            for f in pics:
                ...
                print(f, '(', width, 'x', height, ')', 'Block Size:', block_size, '( ~Est MB:', output_size, ')')
        print('')
        print('Types: x1, x2, x8 (Use x2 as x8 is blocky)')
        print('Types: comp (Compression), up (9x9 Upscale)')
        v1 = input('Type: ')
        if v1 == 'up':
            n_size = input('Input 3, 5, 7, 9 ect for upscale size: ')
        v3 = ('_' + v1 + '.png')
        print('')
        batch_optimizer_a = []
        batch_optimizer_b = []
            ...
            with open(f) as file:
                ...
                for y in range(height - 1):  # 0, 1, 2
                    for x in range(width - 1):  # 0, 1, 2
                        block = arr[y:y + 2, x:x + 2]  # 2×2 chunk
                        blocks.append(block)
                ...
                if v1 == 'comp':
                    ...
                    output_w = blocks_wide
                    output_h = blocks_tall
                elif v1 == 'up':
                    os.makedirs(folder, exist_ok=True)
                    #upscale_9x_pure_python(f, (folder + '/' + img_mod))
                    n_size = int(n_size)
                    upscale_numpy(f, (folder + '/' + img_mod), n_size)
                    continue
                if blocks_wide > 5000 or blocks_tall > 5000:
                    ...
                    #print(blocks_wide, blocks_tall)
                    #print('Wi:', wi, 'Hi:', hi)
                    #full_arr = np.zeros((hi, wi, 3), dtype=np.uint8)
                    heightx, lengthx = arr.shape[:2]
                    full_arr = np.zeros(((heightx * 3), (lengthx * 3), 3), dtype=np.uint8)
                    #print(heightx, lengthx)
                    height, length = full_arr.shape[:2]
                    #print(height, length)
                    #print('WA: ', output_w, 'HA: ', output_h)
                    #print('H: ', h, 'W:', w)
                    #print('Len Of Map:', len(map))
                    #print(map)
                    usage = (sys.getsizeof(map) / 1000000)
                    print("Image:", f, '(',lengthx, 'x', heightx,')', '=>', img_mod, '(',length, 'x', height,')', format(usage, ".2f"), 'MB')
                    dic = {}
                    batch_time = {}
                    process_start = time.time()
                    ...
                    '''z1 = sym('z1')
                    #z2 = sym('z2')
                    equation = Eq((len_pixel_items), ((h * z1) * (w * z1)))
                    solution = solve(equation, z1)
                    u = float(abs(solution[0]))
                    print(u)
                    input('Stop')'''
                    ...
                    grid_h = (full_h - 1) // 2
                    #print(arr.shape[:2])
                    #print(full_arr.shape[:2])
                    if v1 == 'x1':
                        desc = ("Map: " + str(len_pixel_items) + " x1 Processing..." + 'Core Using: ' + str(cores))
                        # Batches are given to 8 Cores
                        with Pool(cores) as pl:
                            for batch_result in track(pl.imap_unordered(process_batch_x1, batches), total=len(batches),description=desc):
                                # iox.update(batch_result)
                                for block_name, pm in batch_result.items():
                                    #print(block_name)
                                    block_num = int(block_name.split('_')[1]) - 1
                                    # pm = [a, b, c, d, e, f, g, h, i]
                                    # 202 x 150 picture would be going to 299 x 403 = full_arr.shape[:2]
                                    block_x = block_num % grid_w
                                    block_y = block_num // grid_w
                                    dst_x = block_x * 2
                                    dst_y = block_y * 2
                                    full_arr[dst_y:dst_y + 3,dst_x:dst_x + 3] = (np.asarray(pm, dtype=np.uint8).reshape(3, 3, 3))
                        process_end = time.time()
                        saving_start = time.time()
                        # for step in track(range(1), description="Saving Image & Compressing PNG..."):
                        img = Image.fromarray(full_arr)
                        ...
                        p_time = (abs(process_start - process_end))
                        s_time = (abs(saving_start - saving_stop))
                        print('Processing Time:', format(p_time, ".3f"), 'Saving Time:', format(s_time, ".3f"), 'Batches:', batch_size, 'of', len_pixel_items)
                    ...
                    if v1 == 'x2':
                        ...
                        if blocks_tall % 2 == 0:
                            ...
                            blocks_tall = (blocks_tall - 1)
                        # Image is trimmed to even pixels
                        # 202 x 150
                        blocks_wide = ((blocks_wide + 1) // 2)
                        blocks_tall = ((blocks_tall + 1) // 2)
                        # 101 x 75 (Image is split in half to allow pixels to be x4)
                        output_w = (blocks_wide * 4)
                        output_h = (blocks_tall * 4)
                        # 404 x 300
                        full_arr = np.zeros((output_h, output_w, 3), dtype=np.uint8)
                        ...
                        # Batches are given to 8 Cores
                        with Pool(cores) as pl:
                            for batch_result in track(pl.imap_unordered(process_batch_x2, batches), total=len(batches),
                                                      description=desc):
                                # iox.update(batch_result)
                                for block_name, pm in batch_result.items():
                                    block_num = int(block_name.split('_')[1]) - 1
                                    block_x = ((block_num % blocks_wide) * 4)
                                    block_y = ((block_num // blocks_wide) * 4)
                                    full_arr[block_y:block_y + 4, block_x:block_x + 4] = np.array(pm,
                        ...
                        p_time = (abs(process_start - process_end))
                        s_time = (abs(saving_start - saving_stop))
                        print('Processing Time:', format(p_time, ".3f"), 'Saving Time:', format(s_time, ".3f"),
                              'Batches:', batch_size,
                              'of', len_pixel_items)
                        if len(batch_optimizer_a) == 0:
                            batch_optimizer_a.append(p_time)
